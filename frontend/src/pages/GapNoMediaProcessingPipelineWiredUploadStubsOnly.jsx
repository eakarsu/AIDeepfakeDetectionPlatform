...
/**
 * Gap/CFS Feature Page: No media-processing pipeline wired (upload stubs only).
 * Slug: no-media-processing-pipeline-wired-upload-stubs-only
 * Backend: /api/gap-no-media-processing-pipeline-wired-upload-stubs-only
 */
export default function NoMediaProcessingPipelineWiredUploadStubsOnlyPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-media-processing-pipeline-wired-upload-stubs-only', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No media-processing pipeline wired (upload stubs only).</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No media-processing pipeline wired (upload stubs only).</p>
