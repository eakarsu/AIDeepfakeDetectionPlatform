...
/**
 * Gap/CFS Feature Page: No detection-results database schema.
 * Slug: no-detection-results-database-schema
 * Backend: /api/gap-no-detection-results-database-schema
 */
export default function NoDetectionResultsDatabaseSchemaPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      const res = await fetch('/api/gap-no-detection-results-database-schema', {
        method: 'POST',
        headers: {
  ...
  return (
    <div style={{ padding: 24, maxWidth: 900, margin: '0 auto', color: '#e5e7eb' }}>
      <h2 style={{ marginBottom: 8 }}>No detection-results database schema.</h2>
      <p style={{ opacity: 0.75, marginBottom: 16 }}>No detection-results database schema.</p>
