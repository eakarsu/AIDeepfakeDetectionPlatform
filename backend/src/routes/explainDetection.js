/**
 * Custom Feature: explain-detection
 ...
 * Integration credentials: process.env.FEATURE_EXPLAIN_DETECTION_KEY
...
const FEATURE_KEY = process.env.FEATURE_EXPLAIN_DETECTION_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Explainable deepfake detection.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDeepfakeDetectionPlatform - explain-detection',
    },
    body: JSON.stringify({
...
router.post('/explain-detection', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_EXPLAIN_DETECTION_KEY');
    const ai = await callLLM({ feature: 'explain-detection', goal: 'Explainable deepfake detection', input: payload });
    // No persistence wired.
    return res.json({
      ok: true,
      feature: 'explain-detection',
      endpoint: '/api/ai/explain-detection',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[explain-detection] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/explain-detection/health', (req, res) => {
  res.json({
    feature: 'explain-detection',
    endpoint: '/api/ai/explain-detection',
