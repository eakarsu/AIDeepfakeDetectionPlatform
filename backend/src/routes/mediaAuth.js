...
const FEATURE_KEY = process.env.FEATURE_MEDIA_AUTH_KEY;
const SYSTEM_PROMPT = `You are an expert assistant specialized in: Real-time media authentication.
...
async function callLLM(userPayload) {
  ...
  const response = await fetchFn('https://openrouter.ai/api/v1/chat/completions', {
    ...
    headers: {
      ...
      'X-Title': 'AIDeepfakeDetectionPlatform - media-auth',
    },
    body: JSON.stringify({
...
router.post('/media-auth', auth, async (req, res) => {
  try {
    const payload = req.body || {};
    if (!payload || Object.keys(payload).length === 0) {
      return res.status(400).json({ error: 'Request body is required' });
    }
    if (!FEATURE_KEY) res.set('X-Feature-Credentials-Missing', 'FEATURE_MEDIA_AUTH_KEY');
    const ai = await callLLM({ feature: 'media-auth', goal: 'Real-time media authentication', input: payload });
    // No persistence wired.
    return res.json({
      ok: true,
      feature: 'media-auth',
      endpoint: '/api/ai/media-auth',
      ai_result: ai.result,
      model: ai.model,
      tokens: ai.tokens,
    ...
    console.error('[media-auth] error:', err.message);
    return res.status(err.statusCode || 500).json({ error: err.message || 'Internal error' });
  }
});

router.get('/media-auth/health', (req, res) => {
  res.json({
    feature: 'media-auth',
    endpoint: '/api/ai/media-auth',
